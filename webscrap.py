#All Libraries
import pandas as pd #panda for dat handling and dataframing 
...
def select():
   ...
   while run == True :      #main loop for select
        ...
        if n==1 :
          ...
          print("------------------------------------------------------------------")
          print("Team : ",teamnum,emoji.emojize(":group:"))
          print("Total Goals : ",t_goals,emoji.emojize(":soccer:"))
          print("Winrate : ",round(winr,2),emoji.emojize(":crown:"))
          print("Shotrate : ",round(t_shotrate,2),emoji.emojize(":fire:"))
          print("Taken Goals : ",h_def,emoji.emojize(":shield:"))
          ...
          print("------------------------------------------------------------------")
           ...
           print(f"[bold cyan]{top_t8}  is the Champion Team. We can see thier prgress throughout season making the pattern of victory[/bold cyan]",emoji.emojize(":Crown:"))
           ...
           h=df[['HomeTeam','FTR']].copy()
           a=df[['AwayTeam','FTR']].copy()
           h.columns=['Team','R']
           a.columns=['Team','R']
           h['P']=h["R"].map({'H':'W','D':'D','A':'L'})
           a['P']=a["R"].map({'H':'L','D':'D','A':'W'})

         #   t=pd.concat([h[['Team','R']],a[['Team','R']]])
           t=pd.concat([h,a])
...
def show_menu():
   ...
   table.add_row("9","Choose Team to see stats")
   table.add_row("10","Show whole Table of Teams")
